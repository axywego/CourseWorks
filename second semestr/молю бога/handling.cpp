...
void editUsers(Deque* users) {
    ...
    std::string itemsToEdit[]{ "ВЫБОР ИЗМЕНЕНИЯ","Поменять фамилию", "Поменять имя", "Поменять тип аккаунта",
                               "Поменять интересы", "Поменять друзей" , "Выйти в меню" };
    ...
    startEdit:
        ...
        if (choice == 3) {
            ...
            std::cout << "Вы собираетесь изменить данные этого пользователя:\n\n";
    ...
    if (users->size == 0) {
        ...
        while (true) {
            ...
            if (choice == 3) {
                ...
                std::cout << "Пользователь, которого вы хотите удалить:\n";
                std::cout << user;
                std::cout << "Вы уверены в его удалении?\n1. Да\n2. Нет\n\n";
                while (true) {
                    int choice = validInput(input);
